...
export function Benefits() {
  return (
    <section className="bg-[#F3E9D8] py-24 sm:py-28 lg:py-36">
        ...
        <div className="mt-14 grid border-l border-t border-brown/15 md:grid-cols-3">
          {benefits.map((benefit, index) => (
            <Reveal key={benefit.title} delay={index * 90} className="h-full">
              <article className="group h-full min-h-[290px] border-b border-r border-brown/15 bg-[#FAF6EE]/50 p-7 transition duration-300 hover:bg-[#FAF6EE] sm:p-9">
                <benefit.icon size={32} strokeWidth={1.3} className="text-gold transition group-hover:-translate-y-1" />
                <p className="mt-12 text-[11px] font-bold tracking-[.18em] text-gold">0{index + 1}</p>
                <h3 className="mt-4 font-serif text-2xl font-normal leading-tight text-brown">{benefit.title}</h3>
